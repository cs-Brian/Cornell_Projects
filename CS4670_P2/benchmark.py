...
def plot_2D_arrays(title, arrs, xlabel='', xinterval=None, ylabel='', yinterval=None, line_names=[], aucs=[]):
    ...
    if aucs:
        text = 'Average AUC: {}'.format(np.mean(aucs))
        plt.text(0.5, 0, text, horizontalalignment='center')
...
def benchmark(origImage, trafoImages, homographies,
    ...
    roc_img = plot_2D_arrays(
        ...
        ylabel = 'True rate', line_names=line_legends, aucs=aucs)
